...
function Navbar(props) {
  ...
  return (
    ...
    <div className="flex flex-col justify-between m-2 mx-8 md:flex-row">
        <Topbar />
        <div className="mx-2">
          <Link to={""}>admin@aapkidukaan.live</Link>
        </div>
    </div>
    <div className="flex flex-row m-5 mx-10 justify-between">
      ...
      <div className="mx-5 p-1 whitespace-nowrap">
        ...
        <div className="flex flex-row">
          ...
          <button className="px-1 relative border-transparent text-gray-800 rounded-full hover:text-gray-400 focus:outline-none focus:text-gray-500 transition duration-150 ease-in-out" aria-label="Cart" onClick={
            (e) => {
              e.preventDefault()
              setOpen(true)
            }
          ...
          <Basket />
          <Cart />
          <span className="absolute inset-0 object-right-top -mr-8 mt-3 ml-5 w-5 h-5 rounded-full bg-red flex items-center justify-center">
            <div className="inline-flex items-center px-1.5 py-0.5 text-w border-white rounded-full text-xs font-semibold leading-4 bg-theme text-white">
              {useCart().length}
            </div>
          </span>
        </button>
